...
import { fetchAllProductsInVirtualGarden, fetchProducts, fetchMatchingProducts, updateProductPosition as updateProductPositionThunk,  } from '../thunks/virtualGardenThunks';
...
const potagerVirtuelSlice = createSlice({
  ...
  reducers: {
    ...
    updateProductPosition: (state, action: PayloadAction<{ product_id: number, position: string }>) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAllProductsInVirtualGarden.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message || 'Failed to fetch virtual garden products';
      })
...
export const { addToGarden, updateProductPosition, addToVirtualGarden } = potagerVirtuelSlice.actions;
